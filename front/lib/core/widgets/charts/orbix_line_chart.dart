...
class OrbixLineChart extends StatelessWidget {
  const OrbixLineChart({super.key, required this.points, this.currency = true});
  ...
  String _formatValue(double v) => currency ? formatMoney(v) : compactNumber(v);

  String _formatAxis(double v) => currency ? compactMoney(v) : compactNumber(v);
  ...
  Widget build(BuildContext context) {
    ...
    return ChartSemantics(
      resumo: resumoSerie(
        currency ? 'Evolução do valor' : 'Evolução da quantidade',
        [for (final p in points) (data: p.date, valor: p.value)],
        _formatValue,
      ),
      child: LineChart(
        LineChartData(
