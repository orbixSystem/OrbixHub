...
/// O motivo, limpo — ou nada, quando ninguém escreveu um.
String? _motivo(String? m) {
  final t = m?.trim() ?? '';
...
class BloqueioTotalView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // O MOTIVO manda no bloco em destaque: é a explicação de por que a pessoa
    // está nesta tela, e é o MESMO texto que ela recebeu por e-mail. A data só
    // aparece quando não há motivo gravado (bloqueio anterior a este campo).
    final motivo = _motivo(me.assinatura?.motivo);
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _Moldura(
      ...
      acoes: [
        ...
        const SizedBox(height: 10),
        const FalarComSuporte(assunto: 'Sistema em modo consulta'),
      ],
    );
  }
}
