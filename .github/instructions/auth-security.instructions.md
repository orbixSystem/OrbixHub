---
applyTo: "back/src/modules/auth/**,back/src/common/auth/**,back/src/common/crypto/**"
description: "Segurança de identidade: JWT, refresh rotation, argon2, anti-enumeração."
---

# Segurança de Auth & Identidade

## Tokens

- **Access token**: JWT **HS256** assinado com `JWT_ACCESS_SECRET` (≥ 32 bytes).
  `AccessTokenService.verify` usa **allowlist de algoritmos** `['HS256']` → rejeita
  `alg:none` e confusão de chave assimétrica. TTL padrão 15m.
...
- **Refresh token**: opaco, persistido como **hash** em `refresh_token`. Rotação por família:
  ...
  - **reuso de um token já rotacionado** (fora da tolerância) **revoga a família inteira**
    (proteção contra replay). Lógica em `RefreshService`.

## Senhas (`PasswordService`)

- Hash com **argon2id**; parâmetros vêm do env (`ARGON_MEMORY_KIB`, `ARGON_TIME_COST`,
  `ARGON_PARALLELISM`).
- `dummyVerify(password)` faz um verify real contra um hash dummy cacheado para igualar o
  custo do caminho "e-mail não existe" (anti-timing).
...
- `forgot-password`: **sempre** retorna `{ ok: true }` (200), exista ou não o e-mail.
...
- `email_verify` e `password_reset`: gera token opaco (`generateOpaqueToken`), persiste o
  **hash** (`hashToken`), TTL `OTT_TTL_MIN` (30 min). Consome com `consumeOneTimeToken`.
- `reset-password` consome o token, atualiza a senha e **revoga todas as sessões**
  (`revokeAllForUser`).
