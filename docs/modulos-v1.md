...
| Estoque / Produtos | `inventory` | contratável _(implementado)_ | **Produtos** (itens com estoque) genéricos — identificados por `sku`/`manufacturerCode`/`barcode`; preços decimais; `currentStock`/`minStock` + filtro "abaixo do mínimo"; campos da vertical via `itemFields` + `attributes` (jsonb, whitelist). **Fluxo código-first** `GET /inventory/lookup?code=` (interno → catálogo EAN via `CatalogProvider` Noop/Cosmos com cache Redis → none). Soft delete (`is_active`). Serviço público `getItem`/`incrementStock`/`decrementStock` pronto p/ a OS ("aponta, não invade"). No plano `trial`. **Serviço (mão de obra) não é deste módulo** — é o módulo `Catálogo de Serviços` (planejado); a aba "Serviço" do front é placeholder até ele existir. |
