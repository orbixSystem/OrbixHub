...
| Estoque / Produtos | `inventory` | contratável _(implementado)_ | **Produtos** (itens com estoque) genéricos — identificados por `sku`/`manufacturerCode`/`barcode`; preços decimais; `currentStock`/`minStock` + filtro "abaixo do mínimo"; campos da vertical via `itemFields` + `attributes` (jsonb, whitelist). **Fluxo código-first** `GET /inventory/lookup?code=` (interno → catálogo EAN via `CatalogProvider` Noop/Cosmos/OpenFoodFacts, com **cache durável `catalog_product`** global de 60 dias — cada GTIN consome a API externa uma vez → none). Soft delete (`is_active`). Serviço público `getItem`/`incrementStock`/`decrementStock` pronto p/ a OS ("aponta, não invade"). No plano `trial`. **Serviço (mão de obra) não é deste módulo** — é o módulo `Catálogo de Serviços` (planejado); a aba "Serviço" do front é placeholder até ele existir. |
